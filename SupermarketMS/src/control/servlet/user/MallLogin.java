...
public class MallLogin extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		out.println("<HTML>");
		out.println("  <HEAD><TITLE>商城登录控制器</TITLE></HEAD>");
		...
		try {
			...
			if (rs.next()) {
				...
				if(request.getParameter("origin").equals("malllogin")){
					response.addHeader("refresh","5;url=\"../mall/login.jsp\"");
					out.print("用户名或密码不正确，请重新输入!<br>若5秒未跳转，请点击<a href=../mall/login.jsp>跳转至当前页面</a>");
				}
			
			if(request.getParameter("origin").equals("startlogin")){
				response.addHeader("refresh","5;url=\"../index.jsp\"");
				out.print("用户名或密码不正确，请重新输入!<br> 如果您是我们的员工或管理员,可自行在个人信息修改板块进行帐号绑定!<br>若5秒未跳转，请点击<a href=../index.jsp>跳转至当前页面</a>");
			}
