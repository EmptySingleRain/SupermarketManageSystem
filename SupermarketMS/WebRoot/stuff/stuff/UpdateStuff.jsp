...
if (!confirm("确认要修改？")) {
	...
	//获取表的对象
	var table = document.getElementById("stufftable");
	var form = document.getElementById("stuffform");
	//这样获取的是一个数组
	...
	//遍历复选框数组  如果被选中ci[i].checked的值就是true
		...
		if(JSONObject!=null){
				form.action="<%=basePath%>servlet/UpdateUser?test=UpdateStuff&jsonarray="+stringjson+"";
				form.submit();
		}
		else
		alert("如果要修改，请选中所在行");
  ...
  <body bgcolor='#e6f1fc' onload="goPage(1,10);">
       ...
       <form id="stuffform" action="<%=basePath%>servlet/UpdateUser?test=UpdateStuff" name="tianjia" method="post" class="nput_control">
       <table id="stufftable" border="1" class="table" style="position:absolute;width:99%;">
       <thead> 
        ...
        List<User> list = um.selectAllStuff(user);
        for(User user1:list)
        {      				
    ...
    <tr><td colspan="5" align='center' style="width:20%"><input type="button" name="xg" value="更新员工" style="width: 80" onclick="updatefm()"></td></tr>
    <tr>
